#!/usr/bin/env python3
"""
SDBS check — documentation validation.
...
def run_check(
    ...
    """Run documentation checks on *docs_root*.
    ...
    validate_only : bool
        Only run link validation (skip sync, citations, section refs).
    cleanup_uncited : bool
        Run citation consistency check with cleanup of uncited BibTeX entries.
    verbose : bool
    ...
    -------
    bool
        True when no issues are found, False otherwise.
    """
    target = str(docs_root)

    if validate_only:
        validate_all_links(target, verbose=verbose)
        return True

    if cleanup_uncited:
        check_citation_consistency(target, cleanup=True, verbose=verbose)
        return True

    # Full suite
    sync_all_links(target)
    print("\n" + "=" * 60 + "\n VALIDATION PHASE\n" + "=" * 60 + "\n")
    validate_all_links(target, verbose=verbose)
    yaml_errors = validate_yaml_relative_paths(target)
    print("\n" + "=" * 60 + "\n CITATION CHECK PHASE\n" + "=" * 60 + "\n")
    check_citation_consistency(target, verbose=verbose)
    print("\n" + "=" * 60 + "\n SECTION REFERENCE CHECK\n" + "=" * 60 + "\n")
    section_result = check_all_section_references(target, verbose=True)

    issues_found = yaml_errors > 0
    if section_result:
        issues_found = issues_found or bool(section_result.get("unused"))
        issues_found = issues_found or bool(section_result.get("broken"))
