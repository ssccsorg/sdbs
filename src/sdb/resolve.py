...
Architecture
  ...
  additional classes in this file and dispatched from ``resolve_all()``.

Usage::

    resolve_all(docs_root, check_only=False)
"""
...
def resolve_all(docs_root: Path, check_only: bool = False) -> bool:
    ...
    ----------
    docs_root:
        Root directory of the documentation site (e.g. ``docs/``).
    check_only:
        When ``True``, run in dry-run mode to detect issues without
        modifying any files.

    Returns
    -------
    ``True`` if no fixes were needed.  ``False`` if at least one fix was
    ...
    for name, resolver in zip(headers, resolvers):
        _, fixes = resolver.resolve_all(docs_root, docs_root, dry_run, verbose=False)
        total_fixes += fixes

    print(f"{'=' * 60}")
    if total_fixes:
        action = "would be needed" if check_only else "applied"
